...
class NotebookHelper {
    constructor(page, activity, contents, filebrowser, menu) {
        ...
        this.getCellCount = async () => {
            ...
            do {
                ...
                scrollHeight = await notebook.evaluate(node => node.scrollHeight - node.clientHeight);
            } while (scrollHeight > previousScrollHeight);
            const lastCell = await notebook.$$('div.jp-Cell >> nth=-1');
            const count = parseInt((_a = (await lastCell[0].getAttribute('data-windowed-list-index'))) !== null && _a !== void 0 ? _a : '0', 10) + 1;
    ...
    async getCell(cellIndex) {
        ...
        let firstIndex = parseInt((_a = (await firstCell.getAttribute('data-windowed-list-index'))) !== null && _a !== void 0 ? _a : '0', 10);
        let lastIndex = parseInt((_b = (await lastCell.getAttribute('data-windowed-list-index'))) !== null && _b !== void 0 ? _b : '0', 10);
        if (cellIndex < firstIndex) {
            // Scroll up
            let scrollTop = (_d = (_c = (await firstCell.boundingBox())) === null || _c === void 0 ? void 0 : _c.y) !== null && _d !== void 0 ? _d : (await notebook.evaluate(node => node.scrollTop - node.clientHeight));
            do {
                await notebook.evaluate((node, scrollTarget) => {
                    node.scrollTo({ top: scrollTarget });
                }, scrollTop);
                ...
                firstIndex = parseInt((_e = (await cells[firstCell].getAttribute('data-windowed-list-index'))) !== null && _e !== void 0 ? _e : '0', 10);
                scrollTop =
                    (_g = (_f = (await cells[firstCell].boundingBox())) === null || _f === void 0 ? void 0 : _f.y) !== null && _g !== void 0 ? _g : (await notebook.evaluate(node => node.scrollTop - node.clientHeight));
            ...
            // Scroll down
            const viewport = await (await notebook.$$('.jp-WindowedPanel-window'))[0].boundingBox();
            ...
            do {
                ...
                lastIndex = parseInt((_h = (await cells[lastCell].getAttribute('data-windowed-list-index'))) !== null && _h !== void 0 ? _h : '0', 10);
                const viewport = await (await notebook.$$('.jp-WindowedPanel-window'))[0].boundingBox();
                scrollHeight = viewport.y + viewport.height;
                // Avoid jitter
                scrollHeight = Math.max(previousScrollHeight + clientHeight, scrollHeight);
            } while (scrollHeight > previousScrollHeight && lastIndex < cellIndex);
        }
        if (firstIndex <= cellIndex && cellIndex <= lastIndex) {
            return (await notebook.$$(`div.jp-Cell[data-windowed-list-index="${cellIndex}"]`))[0];
        }
        else {
            return null;
        }
    ...
    async clickCellGutter(cellIndex, lineNumber) {
        ...
        const cell = await this.getCell(cellIndex);
        const gutters = await cell.$$('.cm-gutters > .cm-gutter.cm-breakpoint-gutter > .cm-gutterElement');
     ...
     * @param cellIndex
     */
    async waitForCellGutter(cellIndex) {
        const cell = await this.getCell(cellIndex);
        if (cell) {
    ...
    async clickCodeGutter(lineNumber) {
        ...
        await panel.waitForSelector('.cm-gutters > .cm-gutter.cm-breakpoint-gutter > .cm-gutterElement', { state: 'attached' });
        const gutters = await panel.$$('.cm-gutters > .cm-gutter.cm-breakpoint-gutter > .cm-gutterElement');
        if (gutters.length < lineNumber) {
            return false;
        }
        await gutters[lineNumber].click();
